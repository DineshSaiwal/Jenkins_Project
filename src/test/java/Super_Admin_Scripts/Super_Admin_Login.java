...
public class Super_Admin_Login extends Base_Class{

	ExtentTest test;
	
	@Test
	void Super_Admin_Login_Test() throws InterruptedException {
		
		Login_Page pg= new Login_Page(driver);
	
		
		test = extent.createTest("Super Admin Login Test").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		test.info(MarkupHelper.createLabel("Test Case Executed On { "+ browserName +" } Browser", ExtentColor.BLUE));
		
		
 		 try {pg.Super_Admin_Mail();
