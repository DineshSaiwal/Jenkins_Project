...
public class Admin_Adding_Team_Member extends Base_Class {

ExtentTest test;
	
	@Test
	void Adding_Team_Member_Test() throws Exception{
		
		Adding_User pg = new Adding_User(driver);
		test = extent.createTest("Adding Team Member Test").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Permission p= new Permission();
		 ...
		 try {pg.SelectRole("Team Member");
		 	test.pass(MarkupHelper.createLabel("Selected Role as Team Member", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Failed To Select Role as Team Member",  ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			test.log(Status.FAIL, "Exception occurred ==> " + e.getMessage());
				}
