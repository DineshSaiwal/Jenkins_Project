...
public class UnArchive_Historical_Proposal_File extends Base_Class{
	
	ExtentTest test;

	@Test
	public void Unarchive_Historical_Proposal_File() throws Exception {

		Archived_Files_Page pg = new Archived_Files_Page(driver);
		test = extent.createTest("UnArchive Historical Proposal File").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Permission p= new Permission();
		 ...
		 if (!p.Is_Admin) {
 		 ...
 		 try {pg.UnArchive_File_Type("Historical Proposal File");
		 	test.pass(MarkupHelper.createLabel("Found the Historical Proposal File To Unarchive", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.skip(MarkupHelper.createLabel("⛔ Skipping test: No Historical Proposal File Found To UnArchive", ExtentColor.ORANGE));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			return; 
				}
