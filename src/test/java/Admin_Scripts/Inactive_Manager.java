...
public class Inactive_Manager extends Base_Class{
	
	ExtentTest test;

	@Test
	public void Inavtive_Active_Manager() throws Exception {

		User_Active_Inactive pg = new User_Active_Inactive(driver);
		test = extent.createTest("InActive The Active Manager").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Thread.sleep(1000);
		 ...
		 try {pg.GetStatus("Bid Manager", "Active" );
