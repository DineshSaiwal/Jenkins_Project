...
public class UnArchive_Resource_Library extends Base_Class{
	
	ExtentTest test;

	@Test
	public void Unarchive_Resource_Library() throws Exception {

		Archived_Files_Page pg = new Archived_Files_Page(driver);
		test = extent.createTest("UnArchive Resource Library").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Permission p= new Permission();
		 ...
		 if (!p.Is_Admin) {
 		 ...
 		 try {pg.UnArchive_File_Type("Resource Library");
		 	test.pass(MarkupHelper.createLabel("Found the Resource Library Doc To Unarchive", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.skip(MarkupHelper.createLabel("⛔ Skipping test: No Resource Library Found Doc To UnArchive", ExtentColor.ORANGE));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			return; 
				}
