...
public class UnArchive_Historical_Proposal extends Base_Class{
	
	ExtentTest test;

	@Test
	public void Unarchive_Historical_Proposal() throws Exception {

		Archived_Files_Page pg = new Archived_Files_Page(driver);
		test = extent.createTest("UnArchive Historical Proposal").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Permission p= new Permission();
		 ...
		 if (!p.Is_Admin) {
 		 ...
 		 try {pg.UnArchive_File_Type("Historical Proposal");
		 	test.pass(MarkupHelper.createLabel("Found the Historical Proposal To Unarchive", ExtentColor.GREEN));} 
		 catch (Exception e) {
