...
public class Active_Team_Member extends Base_Class{
	
	ExtentTest test;

	@Test
	public void Active_Inavtive_Team_Member() throws Exception {

		User_Active_Inactive pg = new User_Active_Inactive(driver);
		test = extent.createTest("Active The InActive Team Member").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Thread.sleep(1000);
		 ...
		 try {pg.GetStatus("Team Member", "Inactive" );
		 	test.pass(MarkupHelper.createLabel("InActive Team Member Were Found", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.fail(MarkupHelper.createLabel("Did Not Found InActive Team Member",  ExtentColor.RED));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			test.log(Status.FAIL, "Exception occurred ==> " + e.getMessage());
				}
