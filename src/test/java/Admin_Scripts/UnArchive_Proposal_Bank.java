...
public class UnArchive_Proposal_Bank extends Base_Class{
	
	ExtentTest test;

	@Test
	public void Unarchive_Proposal_Bank() throws Exception {

		Archived_Files_Page pg = new Archived_Files_Page(driver);
		test = extent.createTest("UnArchive Proposal Bank").assignAuthor("Dinesh_Saiwal").assignDevice(browserName).assignCategory("Smoke");
		...
		Permission p= new Permission();
		 ...
		 if (!p.Is_Admin) {
 		 ...
 		 try {pg.UnArchive_File_Type("Proposal Bank");
		 	test.pass(MarkupHelper.createLabel("Found the Proposal Bank Doc To Unarchive", ExtentColor.GREEN));} 
		 catch (Exception e) {
			test.skip(MarkupHelper.createLabel("⛔ Skipping test: No Proposal Bank Found Doc To UnArchive", ExtentColor.ORANGE));
			test.addScreenCaptureFromBase64String(Utility_Class.Capture_Screenshot()); 
			return; 
				}
